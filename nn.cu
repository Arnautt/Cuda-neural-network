...
// Function to catch error when call to cuda API
...
// Multiply 2 matrices d_m1 and d_m2 on device and transfer the result to CPU
void nnet::matrix_mul_gpu2(double *d_m1, double *d_m2, double *d_res, double *h_res, const int h1, const int w1, const int h2, const int w2) {
  // Matrix multiplication on GPU
  // We need ceil function to deal with any matrix dimension (not only factor of block_size)
  // nb_loop variable is the number of tiled matrix we need to get the final result
  dim3 nb_threads(block_size,block_size,1);
...
nnet::nnet(const int n, const int d, const int dim_h, const int niter, const double lr, const int freq_show): freq_show(freq_show), n(n), d(d), dim_h(dim_h), niter(niter), lr(lr) {
  ...
  w2 = new double[dim_h];
  std::cout << "Neural network object created" << std::endl;
  return;
}
...
nnet::~nnet() {
  ...
  delete [] w1;
  delete [] w2;
  std::cout << "Neural network object destructed" << std::endl << std::endl;
}
...
void nnet::random_init() {
  ...
  for (int i = 0 ; i < n ; i++) {
    ...
    y[i] = 0;
    if ( (threshold > 1) ) {y[i] = 1; nb_one++;}
  }

  std::cout << "Dataset information : " << std::endl;
  std::cout << "- Percentage of positive example : " << (nb_one/n)*100 << std::endl;
  std::cout << "- Number of training example : " << n << std::endl;
  std::cout << "- Dimension of the problem : " << d << std::endl;
  std::cout << "- Number of hidden units : " << dim_h << std::endl;
...
double l2_norm(double *vec, const int size) {
  double res = 0.;
  for (int i = 0 ; i < size ; i++) {
    res += pow(vec[i], 2);
  }
...
// Matrix multiplication on CPU : take 2 matrices m1 and m2 and
...
void nnet::train_on_gpu() {
  ...
  // we need theses matrices on GPU to avoid transfer
  double *d_X, *d_w1, *d_z1, *d_X_transpose, *d_delta1, *d_grad_w1;
  CHECK_CUDA_ERROR(cudaMalloc(&d_X, n*d*sizeof(double)));
  CHECK_CUDA_ERROR(cudaMalloc(&d_w1, d*dim_h*sizeof(double)));
  CHECK_CUDA_ERROR(cudaMalloc(&d_z1, n*dim_h*sizeof(double)));
  CHECK_CUDA_ERROR(cudaMalloc(&d_X_transpose, d*n*sizeof(double)));
  CHECK_CUDA_ERROR(cudaMalloc(&d_delta1, n*dim_h*sizeof(double)));
  CHECK_CUDA_ERROR(cudaMalloc(&d_grad_w1, d*dim_h*sizeof(double)));
  ...
  // Copy values to GPU
  CHECK_CUDA_ERROR(cudaMemcpy(d_X, X, n*d*sizeof(double), cudaMemcpyHostToDevice));
  CHECK_CUDA_ERROR(cudaMemcpy(d_w1, w1, d*dim_h*sizeof(double), cudaMemcpyHostToDevice));
  CHECK_CUDA_ERROR(cudaMemcpy(d_X_transpose, X_transpose, d*n*sizeof(double), cudaMemcpyHostToDevice));
  ...
  for (int iter = 0 ; iter < niter ; iter++) {
    ...
    matrix_mul_gpu2(d_X, d_w1, d_z1, z1, n, d, d, dim_h);
    sigmoid(z1, a1, n*dim_h);
    ...
    matrix_mul(delta2, w2, trans, n, 1, 1, dim_h);
    calculate_delta1(z1, delta1, trans, n, dim_h);
    CHECK_CUDA_ERROR(cudaMemcpy(d_delta1, delta1, n*dim_h*sizeof(double), cudaMemcpyHostToDevice));
    matrix_mul_gpu2(d_X_transpose, d_delta1, d_grad_w1, grad_w1, d, n, n, dim_h);
    delete [] trans;
  ...
  // Free GPU Memory
  cudaFree(d_X); cudaFree(d_w1); cudaFree(d_z1);
  cudaFree(d_X_transpose); cudaFree(d_delta1); cudaFree(d_grad_w1);
